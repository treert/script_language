...
#define METATABLE_FILE "file"
    ...
    int PushError(oms::StackAPI &api)
    {
        api.PushNil();
    ...
    void ReadBytes(oms::StackAPI &api, std::FILE *file, int bytes)
    {
        if (bytes <= 0)
            api.PushString("");
        else
    ...
    void ReadByFormat(oms::StackAPI &api, std::FILE *file,
    ...
    int Close(oms::State *state)
    {
        oms::StackAPI api(state);
        if (!api.CheckArgs(1, oms::ValueT_UserData))
            return 0;
    ...
    int Seek(oms::State *state)
    {
        oms::StackAPI api(state);
        if (!api.CheckArgs(1, oms::ValueT_UserData,
                           oms::ValueT_String, oms::ValueT_Number))
            return 0;
    ...
    int Setvbuf(oms::State *state)
    {
        oms::StackAPI api(state);
        if (!api.CheckArgs(2, oms::ValueT_UserData,
                           oms::ValueT_String, oms::ValueT_Number))
            return 0;
    ...
    int Write(oms::State *state)
    {
        ...
        for (int i = 1; i < params; ++i)
        {
            ...
            else if (type == oms::ValueT_Number)
            {
            ...
            {
                api.ArgTypeError(i, oms::ValueT_String);
                return 0;
            }
        }
    ...
    int Open(oms::State *state)
    {
        oms::StackAPI api(state);
        if (!api.CheckArgs(1, oms::ValueT_String, oms::ValueT_String))
            return 0;
    ...
    void RegisterLibIO(oms::State *state)
    {
        oms::Library lib(state);
        oms::TableMemberReg file[] = {
            { "close", Close },
            { "flush", Flush },
            { "read", Read },
            { "seek", Seek },
